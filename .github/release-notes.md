...
AgentBro is a native macOS desktop tool for AI coding agents. This update continues polishing the first open-source release so installation, updates, troubleshooting, and core capabilities are easier to understand.

### Highlights

- Fixed stable release packaging by ensuring `Contents/Resources/agentbro-bridge` keeps executable permissions and receives Developer ID signing, preventing Apple notarization from rejecting the helper executable.
- Keeps the v0.1.8/v0.1.9 improvements: CLI agent detection, bridge resource bundling, hook installation flow, update restart flow, privacy settings entry, and the in-repo release skill.
